...
export const claimPrize = async (req, res) => {
  ...
  try {
    ...
    // 1️⃣ Obtener detalles del premio y bloquear la fila PRIMERO
    ...
    // 2️⃣ Validar stock disponible antes de continuar
    if (stock <= 0) {
      await connection.rollback();
      return res.status(400).json({ message: "El premio ya no está disponible." });
    }

    // 3️⃣ Obtener puntos del usuario y bloquear la fila
    ...
    // 5️⃣ Reducir el stock del premio usando UPDATE con condición
    ...
    res.status(200).json({
      ...
      prize: { id: prizeId, name: prizeName, updatedStock: stock - 1 },
    });
  } catch (error) {
    await connection.rollback();
    console.error("Error al reclamar premio:", error);
    res.status(500).json({ message: "Error al reclamar el premio." });
  } finally {
    connection.release();
  }
};
